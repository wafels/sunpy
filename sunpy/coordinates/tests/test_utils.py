...
def test_great_arc_coordinates(points_requested, points_expected, first_point,
    ...
    print(points_requested, points_expected)
    ...
    # Starting co-ordinate in Cartesian co-ordinates
    np.testing.assert_almost_equal(gc.cartesian_coordinates[0], np.asarray([428721.09135385, -428722.90519236, 341776.09102756]))

    # Ending co-ordinate in Cartesian co-ordinates
    np.testing.assert_almost_equal(gc.cartesian_coordinates[-1], np.asarray([-71429.52293813, 571439.07124801, 390859.57978693]))

    # Center co-ordinate  in Cartesian co-ordinates
    ...
    # Third vector - lies in the plane formed by the first and second vector
    ...
    # In these tests, all the co-ordinates are on the visible disk
    assert np.all(gc.front_or_back) is True
    assert gc.all_on_front is True
    assert gc.all_on_back is False
    assert gc.from_front_to_back_points_index is None
    assert gc.from_back_to_front_points_index is None
    np.testing.assert_almost_equal(gc.front_arc_indices, np.arange(0, gc._npoints))
    assert gc.back_arc_indices is None
